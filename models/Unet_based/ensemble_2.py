...
def merging_network(output_1, output_2):
    tensor = Concatenate([output_1, output_2])
    tensor = Conv2D(128, (3, 3), padding="same")(tensor)
    tensor = BatchNormalization()(tensor)
    tensor = Activation("relu")(tensor)

    tensor = Conv2D(64, (3, 3), padding="same")(tensor)
    tensor = BatchNormalization()(tensor)
    output = Activation("relu")(tensor)
...
def build_model(input_size=256, num_blocks=3):
    ...
    aux_network = merging_network(output_1, output_2)
    output_layer = Activation("sigmoid")(aux_network)
