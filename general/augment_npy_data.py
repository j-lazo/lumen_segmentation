...
def augment_data(files_path):
    ...
    for i, element in enumerate(files[:]):
        ...
        img = np.load("".join([files_path, 'image/', element]))
        mask = np.load("".join([files_path, 'label/', element]))
        ...
        # save the images
        np.save("".join([files_path, 'image/', element[:-4], '_1', '.png']), im_rot1)
        np.save("".join([files_path, 'image/', element[:-4], '_2', '.png']), im_rot2)
        np.save("".join([files_path, 'image/', element[:-4], '_3', '.png']), im_rot3)
        np.save("".join([files_path, 'image/', element[:-4], '_4', '.png']), horizontal_img)
        np.save("".join([files_path, 'image/', element[:-4], '_5', '.png']), vertical_img)

        np.save("".join([files_path, 'label/', element[:-4], '_1', '.png']), mask_rot1)
        np.save("".join([files_path, 'label/', element[:-4], '_2', '.png']), mask_rot2)
        np.save("".join([files_path, 'label/', element[:-4], '_3', '.png']), mask_rot3)
        np.save("".join([files_path, 'label/', element[:-4], '_4', '.png']), horizontal_mask)
        np.save("".join([files_path, 'label/', element[:-4], '_5', '.png']), vertical_mask)
        ...
        for i in range(4):
            ...
            np.save("".join([files_path, 'image/', element[:-4], '_', str(i + 6), '.png']), image_brg)
            np.save("".join([files_path, 'label/', element[:-4], '_', str(i + 6), '.png']), mask_choice)
        ...
        np.save("".join([files_path, 'image/', element[:-4], '_10_.png']), zoom_in_img)
        np.save("".join([files_path, 'label/', element[:-4], '_10_.png']), zoom_in_mask)
        ...
        np.save("".join([files_path, 'image/', element[:-4], '_11_.png']), zoom_out_img)
        np.save("".join([files_path, 'label/', element[:-4], '_11_.png']), zoom_out_mask)


def main():
    path_directory = '/home/nearlab/Jorge/current_work/lumen_segmentation/data/3x3_grayscale_dataset/val/augmented_data/'
