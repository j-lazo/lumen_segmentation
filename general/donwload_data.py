...
if __name__ == "__main__":
    ...
    if os.path.isdir(fileName.split('.')[0]):
        ...
        move(os.path.join(fileName.split('.')[0], 'test_generalization_EAD2019'),
    ...
    l_detection = len(
        glob.glob(os.path.join(testImages.split('.')[0], 'test_images_detection', '*.jpg'), recursive=True))
    l_semantic = len(glob.glob(os.path.join(testImages.split('.')[0], 'test_images_semantic', '*.jpg'), recursive=True))
    l_generalization = len(
        glob.glob(os.path.join(testImages.split('.')[0], 'test_generalization', '*.jpg'), recursive=True))
